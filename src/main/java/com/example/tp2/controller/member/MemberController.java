	...
	public String regist(@ModelAttribute Member member, BindingResult bindingResult) {
		...
		if (memberService.findByUserId(member.getUserId()).isPresent()) {
			bindingResult.addError(
				new FieldError("member", "userId", member.getUserId(), false, null, null, "이미 중복된 아이디가 존재합니다."));
			log.info("userId={}", member.getUserId());
		}
		if (member.getPassword().length() < 8) {
			bindingResult.addError(
				new FieldError("member", "password", member.getPassword(), false, null, null, "비밀번호는 8자리 이상이어야 합니다. "));
		}
